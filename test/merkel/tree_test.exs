defmodule Merkel.TreeTest do
      ...
      root = 
        case k1 > k2 do 
          true -> 
            ...
            %Node{key_hash: root_hash, search_key: k2, key: nil, value: nil, height: 1, left: k2_node, right: k1_node}
            
          false -> 
            root_hash = hash_concat(k1_hash, k2_hash) 
            %Node{key_hash: root_hash, search_key: k1, key: nil, value: nil, height: 1, left: k1_node, right: k2_node}
  ...
  describe "tree of size 2" do
    ...
    test "crud", %{size: _sz, trees: {t0, t1, t2}, valid_keys: vkeys, invalid_keys: ikeys, valid_values: vvs} do
      ...
      k3_hash = hash(k3)
      k3_node = %Node{key_hash: k3_hash, search_key: k3, key: k3, value: v3, height: 0, left: nil, right: nil}

      root = 
        case k3 > root.search_key do 
          true -> 
            # Logger.debug("root.right is #{inspect root.right}")
            inner = if k3 > root.right.search_key do
              h = hash_concat(root.right.key_hash, k3_hash) 
              # Create new inner node
              %Node{key_hash: h, search_key: root.right.key, key: nil, value: nil, height: 1, left: root.right, right: k3_node}
            else
              h = hash_concat(k3_hash, root.right.key_hash) 
              # Create new inner node
              %Node{key_hash: h, search_key: k3, key: nil, value: nil, height: 1, left: k3_node, right: root.right}
            end
          ...
          false -> 
            ...
            inner = if k3 > root.left.search_key do
              h = hash_concat(root.left.key_hash, k3_hash) 
              # Create new inner node
              %Node{key_hash: h, search_key: root.left.key, key: nil, value: nil, height: 1, left: root.left, right: k3_node}
            else
              h = hash_concat(k3_hash, root.left.key_hash) 
              # Create new inner node
              %Node{key_hash: h, search_key: k3, key: nil, value: nil, height: 1, left: k3_node, right: root.left}
            end
